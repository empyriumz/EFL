...
def logdet_grad(op, grad):
    a = op.inputs[0]
    a_adj_inv = tf.debugging.check_numerics(
                    tf.linalg.inv(a, adjoint=True), 
                    'zero determinant')
    out_shape = tf.concat([tf.shape(input=a)[:-2], [1, 1]], axis=0)
    return tf.reshape(grad, out_shape) * a_adj_inv
    
# define logdet by calling numpy.linalg.slogdet
def logdet(a, name = None):
    with tf.compat.v1.name_scope(name, 'LogDet', [a]) as name:
        res = tf.debugging.check_numerics(
...
class RegionServer(object):
    ...
    def representative(self):
        ...
        # check for cache
        if hasattr(self, 'representative_cache'): # if cache exist
            # use cache
            for region in self.representative_cache:
                yield region
        else: # if cache not established, built it
...
#     size :: int : length of the 1D many-body state
    ...
    def S(self, sites):
        ...
        S = -self.c * np.sum(np.log(np.abs(p**2 + (1.- p)**2))).item()
        return S
...
class IsingModel(object):
    ...
    def build(self, lnD):
        ...
        with tf.compat.v1.name_scope('h'):
        ...
        with tf.compat.v1.name_scope('Ising_net'):
        ...
        with tf.compat.v1.name_scope('free_energy'):
        ...
        self.Ssys = tf.compat.v1.placeholder(dtype = tf.float64, shape = [None], name = 'S_sys')
        with tf.compat.v1.name_scope('cost'):
            ...
            tf.compat.v1.summary.scalar('logMSE', tf.math.log(self.MSE))
            tf.compat.v1.summary.scalar('logwall', tf.math.log(self.wall + 1.e-10))

        # coupling regularizer
        with tf.compat.v1.name_scope('regularizer'):
    ...
    def free_energy(self, A, J, h, name = 'F'):
        with tf.compat.v1.name_scope(name):
            with tf.compat.v1.name_scope('Jh'):
...
machine = Machine(
    ...
    FreeFermion(size=32,mass=1e-8,c=8.),method='weighted')

for i in range(5):
    machine.train(6, learning_rate=0.002, beta1=0.9)
